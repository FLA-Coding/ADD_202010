...
achatsLabel = Label(tkWindow, text="Déplacements pour se rendre dans un établissement culturel autorisé ou un lieu de culte ; déplacements pour effectuer des achats de biens, pour des services dont la fourniture est autorisée, pour les retraits de commandes et les livraisons à domicile", wraplength=600).grid(sticky='e', row=8, column=0) 
achats = IntVar()
Checkbutton(tkWindow, variable=achats).grid(row=8, column=1)

#sante tick
santeLabel = Label(tkWindow, text="Consultations, examens et soins ne pouvant être assurés à distance et achats de médicaments", wraplength=600).grid(sticky='e', row=9, column=0) 
sante = IntVar()
Checkbutton(tkWindow, variable=sante).grid(row=9, column=1)

#famille tick
familleLabel = Label(tkWindow, text="Déplacements pour motif familial impérieux, pour l’assistance aux personnes vulnérables et précaires ou la garde d’enfants", wraplength=600).grid(sticky='e', row=10, column=0) 
famille = IntVar()
Checkbutton(tkWindow, variable=famille).grid(row=10, column=1)

#handicap tick
handicapLabel = Label(tkWindow, text="Déplacements des personnes en situation de handicap et leur accompagnant", wraplength=600).grid(sticky='e', row=11, column=0) 
handicap = IntVar()
...
#convocation tick
convocationLabel = Label(tkWindow, text="Convocations judiciaires ou administratives et déplacements pour se rendre dans un service public", wraplength=600).grid(sticky='e', row=13, column=0) 
convocation = IntVar()
...
#enfants tick
enfantsLabel = Label(tkWindow, text="Déplacements pour chercher les enfants à l’école et à l’occasion de leurs activités périscolaires", wraplength=600).grid(sticky='e', row=15, column=0) 
enfants = IntVar()
