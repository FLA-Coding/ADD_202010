...
#travail tick
travailLabel = Label(tkWindow, text="Déplacements entre le domicile et le lieu d'exercice de l'activité professionnelle ou un établissement d'enseignement ou de formation, déplacements professionnels ne pouvant être différés, déplacements pour un concours ou un examen.", wraplength=600).grid(sticky='e', row=7, column=0) 
travail = IntVar()
Checkbutton(tkWindow, variable=travail).grid(row=7, column=1)

#achats tick
achatsLabel = Label(tkWindow, text="Déplacements pour effectuer des achats de fournitures nécessaires à l'activité professionelle, des achats de première nécessité dans des établissements dont les activités demeurent autorisées, le retrait de commande et les livraisons à domicile.", wraplength=600).grid(sticky='e', row=8, column=0) 
achats = IntVar()
Checkbutton(tkWindow, variable=achats).grid(row=8, column=1)

#sante tick
santeLabel = Label(tkWindow, text="Consultations, examens et soins ne pouvant être ni assurés à distance ni différés et l'achat de médicaments.", wraplength=600).grid(sticky='e', row=9, column=0) 
sante = IntVar()
Checkbutton(tkWindow, variable=sante).grid(row=9, column=1)

#famille tick
familleLabel = Label(tkWindow, text="Déplacement pour motif familial impérieux, pour l'assistance aux personnes vulnérables et précaires ou la garde d'enfants.", wraplength=600).grid(sticky='e', row=10, column=0) 
famille = IntVar()
Checkbutton(tkWindow, variable=famille).grid(row=10, column=1)

#handicap tick
handicapLabel = Label(tkWindow, text="Déplacement des personnes en situation de handicap et leur accompagnant.", wraplength=600).grid(sticky='e', row=11, column=0) 
handicap = IntVar()
Checkbutton(tkWindow, variable=handicap).grid(row=11, column=1)

#sport_animaux tick
sport_animauxLabel = Label(tkWindow, text="Déplacements brefs, dans la limite d'une heure quotidienne et dans un rayon maximal d'un kilomètre autour du domicile, liés soit à l'activité physique individuelle des personnes, à l'exclusion de toute pratique sportive collective et de toute proximité avec d'autres personnes, soit à la promenade avec les seules personnes regroupées dans un même domicile, soit aux besoins des animaux de compagnie.", wraplength=600).grid(sticky='e', row=12, column=0) 
sport_animaux = IntVar()
Checkbutton(tkWindow, variable=sport_animaux).grid(row=12, column=1)

#convocation tick
convocationLabel = Label(tkWindow, text="Convocation judiciaire ou administrative et pour se rendre dans un service public", wraplength=600).grid(sticky='e', row=13, column=0) 
convocation = IntVar()
Checkbutton(tkWindow, variable=convocation).grid(row=13, column=1)

#missions tick
missionsLabel = Label(tkWindow, text="Participation à des missions d'intérêt général sur demande de l'autorité administrative", wraplength=600).grid(sticky='e', row=14, column=0) 
missions = IntVar()
Checkbutton(tkWindow, variable=missions).grid(row=14, column=1)

#enfants tick
enfantsLabel = Label(tkWindow, text="Déplacement pour chercher les enfants à l'école et à l'occasion de leurs activités périscolaires.", wraplength=600).grid(sticky='e', row=15, column=0) 
enfants = IntVar()
